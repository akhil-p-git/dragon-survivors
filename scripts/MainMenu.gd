...
func _ready():
	...
	# Shop button
	var shop_btn = _create_menu_button("Upgrades Shop")
...
func _build_character_select() -> Control:
	...
	for char_data in characters:
		var card = _create_char_card(char_data, overlay)
		grid.add_child(card)
...
func _create_char_card(char_data: Dictionary, overlay: Control) -> PanelContainer:
	var unlocked = SaveData.is_character_unlocked(char_data.name)
	...
	# Character sprite
	var sprite_path = "res://assets/sprites/%s.png" % char_data.name
...
func _build_shop() -> Control:
	...
	for upgrade_name in SaveData.upgrade_defs:
		var card = _create_shop_card(upgrade_name, overlay, gold_label, grid)
		grid.add_child(card)
...
func _create_shop_card(upgrade_name: String, overlay: Control, gold_label: Label, grid: GridContainer) -> PanelContainer:
	var def = SaveData.upgrade_defs[upgrade_name]
	var rank = SaveData.get_upgrade_rank(upgrade_name)
	var max_rank = def.max_rank
	var cost = SaveData.get_upgrade_cost(upgrade_name)
	...
	if maxed:
		...
		var can_afford = SaveData.gold >= cost
