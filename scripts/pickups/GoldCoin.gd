...
func _physics_process(delta: float) -> void:
	...
	var attract_range: float = player.pickup_range + 30.0
	if not is_attracted and dist_sq <= attract_range * attract_range:
		is_attracted = true
	if is_attracted:
		...
		if global_position.distance_squared_to(player_pos) < 400.0:  # 20^2
			_collect()
...
static var _gold_texture_cache: Texture2D = null

func _get_gold_texture() -> Texture2D:
	if _gold_texture_cache:
		return _gold_texture_cache
	var path: String = "res://assets/sprites/gold_coin.png"
	if ResourceLoader.exists(path):
		_gold_texture_cache = preload("res://assets/sprites/gold_coin.png")
	else:
		# Fallback: generate a simple gold circle
		var img: Image = Image.create(12, 12, false, Image.FORMAT_RGBA8)
		var center: Vector2 = Vector2(6, 6)
		for x in range(12):
			for y in range(12):
		...
		_gold_texture_cache = ImageTexture.create_from_image(img)
	return _gold_texture_cache
