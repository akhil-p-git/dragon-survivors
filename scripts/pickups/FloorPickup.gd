...
func _process(delta: float) -> void:
	...
	position.y = _base_y + sin(bob_offset) * 4.0
...
func _setup_visual() -> void:
	...
	var path: String = "res://assets/sprites/pickup_%s.png" % pickup_type
...
func _apply_effect(player: CharacterBody2D) -> void:
	match pickup_type:
		"chicken":
			var heal_amount: float = player.max_hp * 0.30
		...
		"magnet":
			...
			for orb in orbs:
				if is_instance_valid(orb) and orb.has_method("force_attract"):
					orb.force_attract()
			var coins: Array[Node] = get_tree().get_nodes_in_group("gold_coins")
			for coin in coins:
...
func _generate_pickup_texture(color: Color) -> Texture2D:
	...
	var center: Vector2 = Vector2(7, 7)
	for x in range(14):
		for y in range(14):
