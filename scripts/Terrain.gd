...
func _draw():
	...
	var base_color = Color(0.157, 0.137, 0.176)  # RGB ~40,35,45
	draw_rect(Rect2(-5000, -5000, 10000, 10000), base_color)

	# Stone tile grid (every 64px) with darker grout lines
	var grout_color = Color(0.086, 0.071, 0.098, 0.6)
	var grout_highlight = Color(0.22, 0.20, 0.25, 0.3)
	...
	var dark_patch = Color(0.10, 0.09, 0.12, 0.35)
	var rng = RandomNumberGenerator.new()
	rng.seed = 12345  # Deterministic
	for i in range(200):
		var px = rng.randf_range(-4500, 4500)
		var py = rng.randf_range(-4500, 4500)
		var size = rng.randf_range(30, 80)
		draw_rect(Rect2(px, py, size, size), dark_patch)

	# Occasional lighter stone highlights
	var light_patch = Color(0.22, 0.20, 0.24, 0.2)
	for i in range(80):
		var px = rng.randf_range(-4500, 4500)
		var py = rng.randf_range(-4500, 4500)
		var size = rng.randf_range(10, 30)
